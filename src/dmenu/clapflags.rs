...
pub fn validate(config: &mut Config) -> CompResult<()> {
    ...
    if CLAP_FLAGS.occurrences_of("return_int") == 1 {
        config.return_int = true;
    }

    if CLAP_FLAGS.occurrences_of("password") == 1 {
    ...
    if let Some(border_width) = CLAP_FLAGS.value_of("border_width") {
        config.border_width = border_width.parse::<u32>().map_err(|_| {
            Die::Stderr("--bw: Border width must be a non-negative integer".to_owned())
        })?;
    }
